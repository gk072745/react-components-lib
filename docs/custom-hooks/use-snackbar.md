---
sidebar_label: useSnackbar
title: useSnackbar
description: A hook for managing snackbar notifications with global state and single notification support.
---

### Location

```
src/customHooks/useSnackbar.js
```
...
- Manages global snackbar notification state (single notification at a time)
- Provides convenience methods for different notification types (success, error, warning, info)
- Allows configuration of position, offset, and timeout
- Auto-dismisses notifications after timeout (unless persistent)
- Supports custom icons and close buttons
- Replaces existing notification when a new one is shown
...
import { useSnackbar } from '@/src/customHooks/useSnackbar';
...
function App() {
  ...
  return (
    <div>
      ...
      <button onClick={() => warning('This action cannot be undone')}>Warning</button>
...
function PositionOverrideExample() {
...
function ConfigExample() {
  const { success, setPosition, setOffset, setDefaultTimeout } = useSnackbar();

  const configureAndShow = () => {
    setPosition('bottom-right');
    setOffset(2, 2);
    setDefaultTimeout(5000);
    success('Snackbar with custom configuration!');
  };

  return (
    <div>
      <BasicSnackbar />
      <button onClick={configureAndShow}>Show Configured Snackbar</button>
    </div>
  );
}
```
