...
title: useToast
description: A hook for managing toast notifications with global state and multiple notification support.
---

### Location

```
src/customHooks/useToast.js
```
...
- Supports multiple notifications at different positions
- Provides convenience methods for different notification types (success, error, warning, info)
- Allows configuration of position, offset, timeout, and max notifications
...
### With custom position and options
...
function CustomToastExample() {
  ...
  const showCustomToast = () => {
    ...
    success('Custom positioned toast!', {
      secondaryMessage: 'This toast has custom settings',
      timeout: 5000,
    });
  };
